...
Need to research when data is stationary vs. not... 
...
``` 
It picked up our p=0, meaning there are no autoregressive terms (the current value is not directly predicted by past raw data values).
...
Another attempt, trying to directly replicate provided code and try other things: 

```{r}
#Modeling counterfactual 
#LM w/ ARIMA(0,0,1) errors 
m_counterfactual <- monthly.ts %>%
  filter(month < make_yearmonth(2021, 9)) %>%
  model(arima_null = ARIMA(Deaths ~ pdq(0,0,1) #Specifies non-seasonal components of the model
                           + PDQ(0,0,0, period = 12))) #specifies seasonal components of the model 

#trying to make stepwise true: 
m_counterfactual <- monthly.ts %>%
  filter(month < make_yearmonth(2021, 9)) %>%
  model(
    arima_null = ARIMA(
...
fc_counterfactual <- m_counterfactual %>%
  forecast(h = 28) 
#Forecasting counterfactual and plotting against actual data
m_counterfactual %>% 
  forecast(h = 28)  %>% 
  autoplot(monthly.ts) +
  geom_vline(xintercept = as.numeric(as_date(make_yearmonth(2021, 9))), 
             linetype = "dashed",
             color = "grey") +
  theme_minimal() +
...
```
Both of these picked up ARIMA(0,1,1) instead of (0,0,1). Perhaps try to incorporate that into the non-seasonality??


```{r}
m_counterfactual <- monthly.ts %>%
  filter(month < make_yearmonth(2021, 9)) %>%
  model(arima_null = ARIMA(Deaths ~ pdq(0,1,1) #Specifies non-seasonal components of the model
                           + PDQ(0,0,1, period = 12)))
