...
Another attempt, trying to directly replicate provided code and try other things (changing the seasonal components, period change, etc)
...
m_counterfactual <- monthly.ts %>%
  ...
  model(arima_null = ARIMA(Deaths ~ pdq(0,0,1) #Specifies non-seasonal components of the model
                           + PDQ(0,0,0, #specifies seasonal components of the model 
                                 period = 12))) #period is number of observations in each seasonal period
...
m_counterfactual <- monthly.ts %>%
  filter(month <  make_yearmonth(2021, 9)) %>%
  model(
    arima_null = ARIMA(Deaths ~ pdq(4,1,0))
  )
#pre-specified
