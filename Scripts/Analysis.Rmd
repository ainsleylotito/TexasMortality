...
There are several studies that use the interupted time series ARIMA, but we are primarily going to replicate the some of the methods described in [Singh et al., 2025](https://link.springer.com/article/10.1007/s00127-025-02902-7), a study which analyzes suicide deaths among reproductive-aged women post-*Dobbs*. 
...
*I am suppressing results because it is lengthy!!!* 
...
### Sensitivity ARIMA fitting 

We are going to check on determining the ARIMA structure using only the pre-intervention period, then refitting the model to all 6 years, forcing the ARIMA structure to stay the same.  We will then re-estimate the step and ramp coefficients using the full data. 
...
Also comparing Ljung-Box: 

```{r}
augment(m_sensitivity) %>% 
  features(.resid, ljung_box)
...
``` 
Both p-values very high, so both models account for serial correlation. 

Difference in step/ramps estimates due to differencing??
...
```{r arima_specs3, results='hide', message = FALSE, warning = FALSE}
# Use automated algorithm to identify p/q parameters
# Specify first difference = 1 and seasonal difference = 1
m_monthly.ts <- monthly.ts %>% 
 model(arima = ARIMA(Deaths ~ step + ramp,
...
It picked up 
**p=1**, meaning there is an autoregressive terms (the current value *is* directly predicted by past raw data values).

**d=0**, meaning there is no differencing; the original, raw data is stationary, possessing no trend. 

**q=0**: there is no moving average term.

The ARIMA estimated an immediate increase in **95 deaths** and  an estimated reduction of **-2.85 deaths** each month after Bill 8 was enacted. 
...
``` 
This is pretty interesting, it appears as though the only points outside of the forecase are the decrease in deaths right at Jan 2022.
...
report(m_monthly.ts)
``` 
This one gave us two different components! 
(0,1,1)(1,0,0) 

It picked up 
**p=0**, meaning there are no autoregressive terms (the current value is not directly predicted by past raw data values).

**d=1**, meaning there is differencing; the original, raw data possesses a trend. 

**q=1**: there is one moving average term, aka the model uses the error from the previous tie step to predict the current value
 
 For seasonality, we got 
 1 autoregressive term.


The ARIMA estimated an immediate increase in **6.95 deaths** and  an estimated reduction of **-2.3 deaths** each month after Bill 8 was enacted. 
...
``` 
No points popping out of forecast.  
...
#Step and ramp variables 
intervention_month <- monthly_m  %>% 
  filter(month_year == "2021-09-01") %>% 
  pull(time)
monthly_m  <- monthly_m  %>%
  mutate(
...
fc_counterfactual <- m_counterfactual %>%
 ...
 autoplot(fc_counterfactual, monthly_m, level = 95) +
  geom_vline(
